...
def handle_rate_limited(error):
    return Markup('<div class="limiter-message">Too many login attempts. Please try again later.</div>'), 429
...
def signup():
    if request.method == "POST":
        ...
        if bool(db.execute("SELECT username FROM users WHERE username = ?", username)): Flashes("usernameTaken") 
        elif bool(db.execute("SELECT email FROM users WHERE email = ?", email)): Flashes("emailTaken")
        elif not password_matches_pattern(password): Flashes("passwordNotSecure")
        else:
            db.execute("INSERT INTO users(username, password, email, is_verified) VALUES(?, ?, ?, ?)", username, hash(password), email, 0)
...
def verify(username, token):
    ...
    if not email:
        ...
        db.execute("UPDATE USERS SET is_verified = ? WHERE username = ?", 1, username)
        if "email" in session: session["email"] = None
...
def update_password():
    if not "username" in session: return redirect(url_for("login"))
    if request.method == "POST":
        username, old_password, new_password = request.form.get("username"), request.form.get("old_password"), request.form.get("new_password")
        if not bool(db.execute("SELECT id FROM users WHERE username = ? AND password = ?", username, hash(old_password))): Flashes("wrongCredentials")
        elif old_password == new_password: Flashes("passwordMatch")
        elif not password_matches_pattern(new_password): Flashes("passwordNotSecure")
        else:
            db.execute("UPDATE users SET password = ? WHERE username = ?", hash(new_password), username)
...
def reset(token):
    ...
    if request.method == "POST":
        username, password = request.form.get("username"), request.form.get("password")
        if not bool(db.execute("SELECT id FROM users WHERE username = ?", username)): Flashes("usernameNotFound")
        elif not password_matches_pattern(password): Flashes("passwordNotSecure")
        else: 
            db.execute("UPDATE users SET password = ? WHERE username = ?", hash(password), username)
            Flashes("successfulPasswordReset")
            return redirect(url_for("index")) 
